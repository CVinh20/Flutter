...
class _VoucherCardState extends State<VoucherCard> {
  ...
  Future<void> _checkIfSaved() async {
    final saved = await _firestoreService.isVoucherSaved(widget.voucher.id);
    if (mounted) {
      setState(() {
        _isSaved = saved;
      });
  ...
  Future<void> _toggleSaveVoucher() async {
    bool success;
    if (_isSaved) {
      success = await _firestoreService.unsaveVoucherForUser(widget.voucher.id);
    } else {
      success = await _firestoreService.saveVoucherForUser(widget.voucher.id);
    }
