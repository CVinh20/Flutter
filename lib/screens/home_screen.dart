...
class HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  Set<String> _favoriteServiceIds = {}; // Cache favorites as Set for faster lookup
  String _searchQuery = '';

  @override
  void initState() {
    super.initState();
    _userId = MongoDBAuthService.currentUser?.id;
    
    // Debug: Check if user is logged in
    if (_userId != null) {
      print('✅ HomeScreen: User logged in - ID: $_userId');
    } else {
      print('⚠️ HomeScreen: No user logged in');
    }
  ...
  Future<void> _loadFavorites() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _favoriteServiceIds = favorites.map((s) => s.id).toSet();
        });
      }
    } catch (e) {
  ...
  /// Build favorite button for service card
  Widget _buildFavoriteButton(Service service) {
    final isFavorite = _favoriteServiceIds.contains(service.id);
    
    return InkWell(
      onTap: _userId == null ? null : () async {
        try {
          final newState = await _dataService.toggleFavoriteService(service.id);
          if (mounted) {
            setState(() {
              if (newState) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                ...
                backgroundColor: newState ? const Color(0xFF0891B2) : Colors.grey.shade700,
                ...
                children: [
                  Container(
                    ...
                    child: CircleAvatar(
                      radius: 32,
                      backgroundColor: const Color(0xFFF8FAFC),
                      backgroundImage: MongoDBAuthService.currentUser?.photoURL != null ? NetworkImage(MongoDBAuthService.currentUser!.photoURL!) : null,
                      child: MongoDBAuthService.currentUser?.photoURL == null 
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          'Xin chào, ${MongoDBAuthService.currentUser?.displayName.split(' ').last ?? 'Guest'}!',
                        ...
                        Text(
                          MongoDBAuthService.currentUser?.email ?? 'Vui lòng đăng nhập',
                          style: TextStyle(
              ...
              child: Stack(
                children: [
                  ...
                  Positioned(
                    top: 12,
                    left: 12,
                    child: _buildFavoriteButton(service),
                  ),
                ],
              ),
            ),
