...
class StylistBookingDetailScreen extends StatefulWidget {
  ...
  const StylistBookingDetailScreen({super.key, required this.booking});
...
class _StylistBookingDetailScreenState
    extends State<StylistBookingDetailScreen> {
  ...
  bool _dataChanged = false; // Track if data has been changed
  ...
  Future<void> _handleCheckIn() async {
    ...
    try {
      ...
      await _dataService.checkInBooking(_currentBooking!.id);
      ...
      setState(() {
        ...
        _currentBooking = _currentBooking!.copyWith(
          ...
          status: 'in_progress', // Chuyển sang Đang thực hiện
        );
      });

      EasyLoading.dismiss();
      EasyLoading.showSuccess('Check-in thành công! Dịch vụ đang thực hiện.');
    } catch (e) {
      EasyLoading.dismiss();
  ...
  Future<void> _updateServiceStatus(String status) async {
    ...
    if (currentStatus == 'in_progress' && status == 'pending') {
      EasyLoading.showInfo(
        'Không thể quay lại trạng thái "Chờ xử lý" sau khi đã bắt đầu thực hiện',
      );
      return;
    }
    ...
    try {
      ...
      // Sử dụng updateServiceStatus để cập nhật trạng thái
      await _dataService.updateServiceStatus(
        _currentBooking!.id,
        status,
      );
      ...
      if (e.toString().contains('permission-denied')) {
        errorMessage =
            'Lỗi: Không có quyền cập nhật. Vui lòng kiểm tra lại quyền truy cập.';
  ...
  Future<void> _saveNotes() async {
    try {
      ...
      // Sử dụng updateStylistNotes để cập nhật ghi chú
      await _dataService.updateStylistNotes(
        _currentBooking!.id,
            ...
            Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.person, color: Color(0xFF0891B2)),
                ),
            ...
            Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.content_cut, color: Colors.purple),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
