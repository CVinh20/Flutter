...
class _QuickBookingScreenState extends State<QuickBookingScreen> {
  ...
  void initState() {
    ...
    // Load user info from MongoDB Auth cache
  ...
  Future<void> _applyVoucher() async {
    ...
    if (currentUser == null) {
      setState(() {
        _voucherError = 'Vui lòng đăng nhập để sử dụng voucher';
      });
      return;
    }

    try {
      final voucher = await _dataService.getVoucherByCode(code);
      ...
      // Validate with backend to check if user already used this voucher
      final orderValue = selectedService!.price;
      try {
        await _dataService.validateVoucher(
          code,
          currentUser.id,
          orderValue,
        );
        ...
        setState(() {
          _voucherError = e.toString().replaceAll('Exception: ', '').replaceAll('Không thể validate voucher: ', '');
        });
        return;
      }
      ...
      if (orderValue < voucher.minOrderValue) {
        setState(() {
          _voucherError =
              'Đơn hàng tối thiểu ${voucher.minOrderValue.toStringAsFixed(0)}đ';
        });
        return;
      }
      ...
      EasyLoading.showSuccess(
        'Áp dụng voucher thành công! Giảm ${discount.toStringAsFixed(0)}đ',
      );
    } catch (e) {
      setState(() {
  ...
  Future<void> _confirmQuickBooking() async {
    ...
    Booking newBooking = Booking(
      ...
      amount: _appliedVoucher != null
          ? selectedService!.price - _discount
    ...
    try {
      final createdBooking = await _dataService.createBooking(newBooking);
      await NotificationService().scheduleBookingNotification(createdBooking);

      // Nếu có voucher, cập nhật số lượng đã sử dụng
      if (_appliedVoucher != null) {
        final user = MongoDBAuthService.currentUser;
        if (user != null) {
          await _dataService.applyVoucher(_appliedVoucher!.id, user.id);
        }
      }
  ...
  Widget _buildVoucherSection() {
    final finalAmount = selectedService != null
        ? selectedService!.price - _discount
        : 0.0;
    ...
    return _selectButton(
      ...
      onTap: () async {
        final Branch? picked = await showModalBottomSheet<Branch>(
          ...
          enableDrag: false,
          builder: (context) =>
              _BranchPicker(dataService: _dataService),
        );
    ...
    return _selectButton(
      ...
      onTap: () async {
        final Service? picked = await showModalBottomSheet<Service>(
          ...
          enableDrag: false,
          builder: (context) =>
              _ServicePicker(dataService: _dataService),
        );
              ...
              child: _selectButton(
                ...
                onTap: () async {
                  ...
                  if (picked != null && mounted) {
                    ...
                    if (mounted) setState(() => selectedDate = picked);
                  }
                },
              ),
            ),
            ...
            Expanded(
              child: _selectButton(
                ...
                onTap: () async {
                  ...
                  if (picked != null && mounted) {
                    await Future.delayed(Duration(milliseconds: 100));
                    if (mounted) setState(() => selectedTime = picked);
                  }
                },
              ),
            ),
        ...
        _selectButton(
          ...
          onTap: () async {
            final Stylist? picked = await showModalBottomSheet<Stylist>(
              ...
              enableDrag: false,
              builder: (context) =>
                  _StylistPicker(dataService: _dataService),
            );
...
class _ServicePicker extends StatefulWidget {
  ...
  const _ServicePicker({required this.dataService});
...
class _ServicePickerState extends State<_ServicePicker>
  ...
  Future<void> _loadData() async {
    try {
      // Load categories
      final categories = await widget.dataService.getCategories();
      if (mounted) {
        setState(() {
            ...
            Expanded(
              child: _categories.isEmpty
                  ? FutureBuilder<List<Service>>(
                      future: widget.dataService.getServices(),
                  ...
                  : TabBarView(
                      ...
                      children: [
                        ...
                        ..._categories.map(
                          (category) => _buildServicesList(
                            _getServicesByCategory(category.id),
                            controller,
                          ),
                        ),
                      ],
...
class _StylistPicker extends StatelessWidget {
  final DataService dataService;
  const _StylistPicker({required this.dataService});
          ...
          children: [
            ...
            Expanded(
              child: FutureBuilder<List<Stylist>>(
                future: dataService.getStylists(),
                builder: (context, snapshot) {
                  if (!snapshot.hasData) {
                    return Center(child: CircularProgressIndicator());
                  }
...
class _BranchPicker extends StatelessWidget {
  final DataService dataService;
  const _BranchPicker({required this.dataService});
          ...
          children: [
            ...
            Expanded(
              child: FutureBuilder<List<Branch>>(
                future: dataService.getBranches(),
                builder: (context, snapshot) {
                  if (!snapshot.hasData) {
                    return Center(child: CircularProgressIndicator());
                  }
