...
class _ManageCategoriesScreenState extends State<ManageCategoriesScreen> {
  ...
  Future<void> _loadCategories() async {
    setState(() {
      // Create a new key to force FutureBuilder to rebuild
      _futureKey = UniqueKey();
      _categoriesFuture = MongoAdminService.fetchCategories(includeInactive: false);
    });
  }
  ...
  Future<void> _addOrUpdateCategory() async {
    ...
    try {
      if (_editingCategory != null) {
        await MongoAdminService.updateCategory(
          _editingCategory!.copyWith(name: _nameController.text.trim()),
        );
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Cập nhật danh mục thành công!')),
        );
      } else {
        await MongoAdminService.createCategory(name: _nameController.text.trim());
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _deleteCategory(String categoryId) async {
    ...
    if (confirmed == true) {
      try {
        await MongoAdminService.deleteCategory(categoryId);
        await _loadCategories();
        ScaffoldMessenger.of(context).showSnackBar(
