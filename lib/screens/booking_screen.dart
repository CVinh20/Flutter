...
class BookingScreen extends StatefulWidget {
  ...
  const BookingScreen({super.key, this.service, this.initialBranch});
...
class BookingScreenState extends State<BookingScreen>
  ...
  List<Stylist>? _stylists; // Cache stylists
  ...
  Future<void> _loadStylists() async {
    try {
      _stylists = await _dataService.getStylists();
      if (mounted) setState(() {});
    } catch (e) {
      print('Lỗi khi tải stylists: $e');
    }
  }
  ...
  Future<void> _confirmBooking(Service service) async {
    ...
    try {
      ...
      // Lên lịch thông báo nhắc nhở trước 30 phút
      try {
        await NotificationService().scheduleBookingNotification(createdBooking);
        print('✅ Scheduled notification for booking ${createdBooking.id}');
      } catch (e) {
        print('⚠️ Could not schedule notification: $e');
      }
      ...
      if (mounted) {
        print('Booking created successfully, showing dialog...');
        ...
        final result = await showDialog<String>(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                  'Đơn đặt lịch của bạn đã được ghi nhận.',
            ...
            actions: [
              TextButton(
                ...
                child: Text(
                  'Đặt lịch tiếp',
                  style: TextStyle(color: Color(0xFF0891B2), fontWeight: FontWeight.bold),
                ),
              ),
              ElevatedButton(
        ...
        if (result == 'continue') {
          // Reset form để đặt lịch tiếp
          _resetForm();
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Bạn có thể đặt lịch mới'),
              backgroundColor: Colors.green,
              behavior: SnackBarBehavior.floating,
